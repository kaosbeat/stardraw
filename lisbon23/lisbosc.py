...
data = cycle([[0,0,"ready",0,0,0,0,0,0,"for"],[0,0,"some",0,0,0,0,0,0,"beats"]])
...
def piviz(address, *args):
    ...
    if state == "rtext":
        if (option in figfontlist == False): option = "big"
...
async def boot():
    ...
    if bootstates["command"] == "init":
        ...
        s.keynote1(bootstates["framewait"])
    elif bootstates["command"] == "keynote2":
        s.keynote2(bootstates["framewait"])
    elif bootstates["command"] == "panel":
        s.panel(bootstates["framewait"])
    elif bootstates["command"] == "coffee":
        s.coffee(bootstates["framewait"], bootstates["noisestep"])
    elif bootstates["command"] == "drinks":
        s.drinks(bootstates["framewait"], bootstates["noisestep"])    
    elif bootstates["command"] == "wait":
        ast.blinkFiglet(15,20,"grabbing data\n","big", 15,20," ",None,0.8,2)
...
async def signal():
    ...
    if signalstates["command"] == "init":
        ...
        s.perspsquaressignal(signalstates["amount"] )
    elif signalstates["command"] == "cube":
        s.cuber()
    elif signalstates["command"] == "lines":
        s.lines(signalstates["amount"] )
    elif signalstates["command"] == "wlines":
        s.lines(signalstates["amount"], words )
    elif signalstates["command"] == "squares":
        s.squares(signalstates["amount"])
...
async def invertcatstage():
    ...
    mask = s.cyclemask(word,invertedcatstates["cyclepoint"],invertedcatstates["cyclewidth"])
    if invertedcatstates["masktype"] == "none":
        mask = " "*len(word)
    buffer = s.invertedHorizontalCat(word, prewords, mask, invertedcatstates["masktype"] , ast.columns, ast.lines)
...
async def textseq():
    ...
    s.text(textstates["x"], textstates["y"],textstates["font"],textstates["word1"],textstates["word2"])
...
async def rtextseq():
    ...
    if textstates["command"] == "clear":
        ast.clearstage()
    s.rtext(textstates["font"],textstates["word1"],textstates["word2"])
    # ast.printonstage(status, 0,0)
    state = "done"
...
async def interseq():
    ...
    s.intersect(intersectstates["size"],intersectstates["framewait"])
    state = "done"
...
async def circle():
    ...
    s.circle(radius, "mid", "mid" , circlestates["noisestep"],circlestates["seed"])
...
async def banner():
    ...
    if bannerstates["command"] == "banner":
        s.banner("banner", bannerstates["x"], ast.lines - bannerstates["y"], bannerstates["noisestep"],bannerstates["seed"], bannerstates["noisechar"],bannerstates["octaves"])
    elif bannerstates["command"] == "logo":
        s.banner("logo", bannerstates["x"], ast.lines - bannerstates["y"], bannerstates["noisestep"],bannerstates["seed"], bannerstates["noisechar"],bannerstates["octaves"])
    elif bannerstates["command"] == "clear":
        s.banner("clear", bannerstates["x"], ast.lines - bannerstates["y"], bannerstates["noisestep"],bannerstates["seed"], bannerstates["noisechar"],bannerstates["octaves"])
