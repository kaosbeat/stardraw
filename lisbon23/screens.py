...
def bootseq(framewait = 5):
    ...
    ast.printFiglet("ALGORAVE ", "big", 10, ast.lines-10)
    time.sleep(framewait)
    ast.printFiglet("ALGORAVE", "big", 20, ast.lines-21)
    ast.printFiglet("A#G@RA$#", "big", 10, ast.lines-10)
    time.sleep(framewait)
    # ast.initstage("clear")
    ast.printFiglet("ALGORAVE ", "big", 10, ast.lines-10)
    ast.printFiglet("ICLC", "big", 10, ast.lines-21)
    time.sleep(framewait)
    ast.printFiglet("2023", "big", 10, ast.lines-21)
    time.sleep(framewait)
    w,h,text1 = ast.figProps("ICLC", "big")
    w,h,text2 = ast.figProps("2023", "big")
    for i in range(10):
        time.sleep(framewait)
        mergestep = ast.mergeFiglets (text1,text2, 0,0,8,21-i)
        ast.printMultilineonstage(mergestep, 10, ast.lines-10)
    ast.scrollFiglet("LIVECODING BEATS", "big", ast.lines-31, 0.01, 2)
    time.sleep(framewait)
    # ast.initstage()
...
def banner(banner,x,y, noisestep=0.1, seed=1337, noisechar=" ", octaves=8):
    ...
    if banner == "logo":
        ...
        # ast.printMultilineonstage(mergedbuffer,x,y)
        ast.printMultilineonstage(combinebuffers(1,1,1,1))

    elif banner == "clear":
        ast.clearstage(noisechar)
...
def combinebuffers(buf1,buf2,buf3,buf4):
    w1 = 45
    w2 = 45
    w3 = 45
    w4 = 45
    ...
    if buf1:
        ...
        b1 = ast.spacesquare(w1,h)
    if buf2:
        b2 = aa.logo2
    else:
        b2 = ast.spacesquare(w2,h)
    if buf3:
        b3 = aa.logo3
    else:
        b3 = ast.spacesquare(w3,h)
    if buf4:
        b4 = aa.logo4
    else:
        b4 = ast.spacesquare(w4,h)
    interbuf1 = ast.mergeFiglets (buf1,buf2,0,0,w1,0)        
    interbuf2 = ast.mergeFiglets (interbuf1,buf3,0,0,w1+w2,0)
    mergedbuffer  = ast.mergeFiglets (interbuf2,buf4,0,0,w1+w2+w3,0)        
